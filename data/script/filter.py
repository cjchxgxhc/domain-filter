...
本脚本负责：下载源 → 提取域名 → 规则组处理 → 调用 gen_rules.py 生成文件 → 输出 stats.json
"""
...
_GEN_RULES    = Path("data/script/gen_rules.py")

# 需要 deduped 数据集的格式（其余用 simple）
_DEDUPED_FORMATS = frozenset({"adblock", "clash", "singbox", "wildcard"})
...
def save_domains(
    ...
    if not simple_domains and not deduped_domains:
        log_error(f"警告：没有域名可保存（{output_dir.name}）", log_file)
        return {}

    if not _GEN_RULES.exists():
        raise FileNotFoundError(f"gen_rules.py 不存在：{_GEN_RULES}")

    stdin_data = (
        "\n".join(sorted(simple_domains))
        + "\n---\n"
        + "\n".join(sorted(deduped_domains))
    )

    try:
        proc = subprocess.run(
            [
                sys.executable, str(_GEN_RULES),
                str(output_dir),
                title,
                description,
                timezone,
        ...
        raise RuntimeError(
            f"gen_rules.py 调用失败（{output_dir.name}）：{e.stderr[:300]}"
        )
...
def process_group(
    ...
    # ── 子域去重 ──────────────────────────────────────────────────────────
    # 只要请求了任意 deduped 格式，就执行去重；否则 deduped == simple
    needs_dedup = any(f in _DEDUPED_FORMATS for f in formats)
    ...
    try:
        format_counts = save_domains(
            ...
            title, description, formats, timezone, log_file,
        )
    except Exception as e:
        log_error(f"gen_rules.py 调用失败 [{cfg_key}]：{e}", log_file)
        format_counts = {}
