# Domain Filter

![Updated](https://img.shields.io/badge/Updated-2026--07--22_03:20:46-brightgreen?style=flat-square)

## 规则列表

- **Ads** — 主要引用 217heidai/adblockfilters、lingeringsound/10007、privacy-protection-tools/anti-AD 项目规则，增添：手机端应用程序广告；删除：白名单域名、proxy_tld（减小大小）。  
  `AdBlock - Clash - Sing-box - QuantumultX - ShadowRocket - Surge - SmartDNS - Domain - Wildcard` - [查看详情](data/rules/ads)
- **Ads Hosts** — 只包含基本规则。  
  `Hosts` - [查看详情](data/rules/ads_hosts)
- **Proxy** — 所有需要被代理分流的域名。  
  `Clash - Sing-box - QuantumultX - ShadowRocket - Surge - Domain - Wildcard` - [查看详情](data/rules/proxy)
- **Direct Fix** — 修复可能被proxy组错误分流的直连域名。  
  `Clash - Sing-box - QuantumultX - ShadowRocket - Surge - Domain - Wildcard` - [查看详情](data/rules/direct_fix)

*2026-07-22 03:20:46 (CST)*

